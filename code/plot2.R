plot2 <- function()
{
  ...
  # same as plot1, just subset it for baltimore city, fips = 24510
  png("plot2.png")
  par(mfrow = c(2,1))
  ...
  title(main = "Graphs showing emissions for Baltimore City from 1999 -> 2008")
  
  # factor on year and mean of emissions
  df <- ddply(NEI, .(year), summarize, mean = mean(Emissions))
  with(df, plot(year, mean, pch=19, col="blue", type = "o", 
